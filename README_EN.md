...
- Fix: Proper GDI+ lifecycle management via RAII (`GdiplusShutdown` guaranteed on process exit)
- CI: Optimized GitHub Actions cache path to `build/_deps` (FetchContent deps only, improves cache hit rate)
