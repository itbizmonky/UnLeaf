...
以下は v1.0.0 には含まれない将来の改善予定項目。

| 項目 | 内容 |
|------|------|
| engine_core ユニットテスト | `DeferredVerifyTimerCallback` / `PersistentEnforceTimerCallback` 等のコアロジックに対するユニットテスト追加。目標カバレッジ 80% |
| DeleteTimerQueueTimer コールバック設計改善 | `trackedCs_` 取得前提を排除した安全なコールバック設計への移行。デッドロックリスクを設計段階で排除する |
