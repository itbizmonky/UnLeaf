...
### v1.1.4 (2026-05-01 / 更新 2026-05-04)
...
**ETW 安定性修正 (2026-05-04)**

| ID | 内容 |
|----|------|
| #5 | **`MatchAnyKeyword` 0x30 → 0**: Windows 11 Build 26200 で `Microsoft-Windows-Kernel-Process` プロバイダのイベントが `keyword=0` を持つため、`MatchAnyKeyword=0x30` (PROCESS\|THREAD) が全イベントを除外していた。`MatchAnyKeyword=0` に変更してコールバック着火を回復 |
| #6 | **ETW バッファ拡張**: `ETW_BUFFER_SIZE_KB` 64→128 KB、`ETW_MIN_BUFFERS` 4→8、`ETW_MAX_BUFFERS` 32→64。`MatchAnyKeyword=0` 時の高流量 (~1,200 events/sec) に対応 |
| #7 | **ConsumerThread 診断ログ強化**: `OpenTraceW` ハンドル値 (INFO)、`ProcessTrace` 入場・退場と経過時間 (INFO)、初回コールバック受信確認 (INFO)、初回イベントの keyword/eventId (DEBUG) をログ出力。ETW 障害の post-hoc 解析を可能にする |
| #8 | **`firstCallbackLogged_` per-session フラグ**: `Start()` 内で `false` にリセット。セッションごとに初回コールバック到達を1回だけ記録し、複数 ETW 再起動でも正確に追跡する |
| #9 | **`ResolveProcessPath` ノイズ抑制**: `QueryFullProcessImageNameW` の `error=31` (ERROR_GEN_FAILURE、システムプロセス) および `error=87` (ERROR_INVALID_PARAMETER、終了済みプロセス) を DEBUG 出力から除外。未知エラーのみログ対象とする |
| #10 | **ログレベル正規化**: `OpenTraceW handle=` / `ProcessTrace enter` / `ProcessTrace exited` を ALERT→INFO に降格（毎起動/停止で出る通常ログのため）。`Lost event detected` を ALERT→DEBUG に降格（~1/sec の構造的ノイズと確認済み） |
