...
| `PERIODIC_FULL_SCAN_INTERVAL` | 60,000 | NORMAL モード周期 `InitialScan()` 間隔 ms (§9.18) |
| `ETW_STALL_CHECK_INTERVAL` | 30,000 | ETW stall 検知チェック間隔 ms (§9.18) |
| `ETW_RESTART_COOLDOWN_MS` | 180,000 | ETW stall 再起動クールダウン ms (§9.18) |
| `ETW_MIN_EVENTS_FOR_CHECK` | 100 | stall 判定を有効にする最低累計イベント数 (起動直後スキップ, §9.18) |
...
**ETW 監視安定化・Windows 11 Update 後 EcoQoS 解除不全対策 (§9.18)**
...
| #2 | `PerformPeriodicMaintenance`: NORMAL モードで `InitialScan()` を 60 s ごとに自動発火 (`PERIODIC_FULL_SCAN_INTERVAL`)。SafetyNet ラウンドロビン (#1 PID リセット) が 10s × 64 PID/tick で 30〜50s 以内に全 PID をカバーするため、InitialScan は ETW 欠損時の descendant 追跡強制補完として 60s 間隔で十分機能する |
| #3 | `[DIAG]` 診断ログに `etwEvents=%u` フィールドを追加 (`processMonitor_.GetEventCount()`)。ETW デリバリ健全性の事後検証を可能にする |
| #4+#5 | ETW stall 検知: 30 s チェック間隔 (`ETW_STALL_CHECK_INTERVAL`) でイベントカウントを比較。ETW イベント頻度はワークロード依存で保証されず、10s は idle 時 (低負荷・スリープ復帰直後) の正常無イベント期間と区別できないため 30s を採用。無増加 + NORMAL + `ETW_MIN_EVENTS_FOR_CHECK` 以上 + ターゲット稼働中 (`HasAnyTargetRunning()`) + 3 分クールダウン経過 (`ETW_RESTART_COOLDOWN_MS`) の全条件成立時に `ProcessMonitor.Stop() → Start()` を自動実行。再起動失敗時は `DEGRADED_ETW` 遷移 |

新規ヘルパー: `bool HasAnyTargetRunning() const` — `Toolhelp32Snapshot` 軽量版 (collectDescendants 不使用、ハンドル開放なし)。`targetNameSet_` または `pathTargetFileNames_` にマッチする exe を 1 件発見次第 `true` を返す。ETW stall ガードの誤再起動防止に使用。
...
| ETW silent drop (chrome のみ不着) | 〜60 秒 | #2 |
| ETW stall (session alive, events 停止) | 〜30 秒 + 3 分 cooldown | #4+#5 |
