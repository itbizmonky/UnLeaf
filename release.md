...
| 6 | **Lost event DEBUG ログ レート制限** — `lastLostLogTime_` (atomic ULONGLONG) による 60 秒デバウンスで最大 1 回/分に制限。構造的ノイズによる DEBUG ログ洪水を抑制 |
| 7 | **`[DIAG]` ログに `etwLost=N` フィールド追加** — `lastDiagLostCount_` スナップショットにより DIAG インターバルごとの lost event 増分 (デルタ) を記録。バースト/スパイク検出を支援 |
