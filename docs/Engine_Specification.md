...
| **CrashHandler** | `crash_handler.h/cpp` | `SetUnhandledExceptionFilter` ベースの MiniDump ライター (opt-in; `[Logging] CrashDump=1`) |
...
図7: プロセス検出 → STABLE 到達シーケンス
  ...
  └──────────┬──────────┘
             ▼
  ┌─────────────────────┐
  │ OnProcessStart()    │   クリティカルフィルタ / ターゲット名照合
  │  - IsTargetName()   │   → EnqueueRequest(ETW_PROCESS_START)
  │  - IsTrackedParent()│   ※ブロッキングOS呼び出し一切なし
  └──────────┬──────────┘
             ▼ (EngineControlLoop で非同期処理)
  ┌─────────────────────┐
...
#### 4.3.1 criticalQueue_ / nonCriticalQueue_ (エンフォースメントリクエストキュー §9.14-A)

| 項目 | 内容 |
|------|------|
| 型 | `std::deque<EnforcementRequest>` × 2 (`criticalQueue_` / `nonCriticalQueue_`) |
| 保護 | `queueCs_` (CriticalSection) |
| 分類 CRITICAL | ETW_PROCESS_START / DEFERRED_VERIFICATION / PERSISTENT_ENFORCE / SAFETY_NET |
| 分類 NON-CRITICAL | ETW_THREAD_START (高頻度・SOFT_LIMIT でドロップ可) |
| 生産者 | ETW コールバック、Timer Queue コールバック |
| 消費者 | EngineControlThread (`ProcessEnforcementQueue`) |
| 通知 | `enforcementRequestEvent_` (Auto-Reset Event、両キュー空→非空遷移時のみ) |
| 上限 | SOFT_LIMIT=4,096 (NON-CRITICAL 個別) / HARD_LIMIT=8,192 (CRITICAL 個別) / TOTAL_LIMIT=8,192 (合計絶対) |
| バースト制限 | CRITICAL は最大 512 件/tick 処理 (残件は次 tick で継続、re-enqueue 不要) |
| TOTAL 超過時 | nonCritical 追い出し → nonCritical 空なら最古 CRITICAL eviction (完全喪失より最古破棄を優先) |

```
図6: Enforcement Queue フロー (§9.14-A 2-queue)
  ...
  EnqueueRequest()
        ...
        │  isCritical = (type != ETW_THREAD_START)
        │  NON-CRITICAL: SOFT_LIMIT or TOTAL_LIMIT → drop (enforcementDropCount_++)
        │  CRITICAL: HARD_LIMIT → drop (criticalDropCount_++)
        │  CRITICAL + TOTAL超過: nonCritical追い出し or 最古CRITICAL eviction
  ...
  ProcessEnforcementQueue()
        │  CSLockGuard(queueCs_)
        │  CRITICAL: 最大512件をdeque先頭からpop
        │  NON-CRITICAL: swap(nonCritical, nonCriticalQueue_)
        │
        ▼
  CRITICAL 優先処理 → NON-CRITICAL (PMR dedup)
        │  DispatchEnforcementRequest(req)
        └──► Phase ハンドラへ
```

- `EnqueueRequest()` は **両キューが空だった場合のみ** `SetEvent()` を呼ぶ。これにより不要なイベントシグナルを抑制する。
- `ProcessEnforcementQueue()` は CRITICAL をバースト制限付きで優先処理し、NON-CRITICAL は PMR arena デデュプ後に処理する。
...
| 生産者 | OS Thread Pool (`OnProcessExit`)、eviction パス (`TrackProcess`) |
| 消費者 | EngineControlThread (`ProcessPendingRemovals`、最大 256 件/tick) |
| 通知 | `hWakeupEvent_` (Auto-Reset Event) |
| 上限 | `MAX_PENDING_REMOVALS` = 4,096 |
| 上限制御 | `pendingQueueSize_` CAS ベース上限ガード (MAX=512)。超過時は `pendingOverflowFlag_` をセットしノードを delete |
| overflow 回復 | `pendingOverflowFlag_` → `HandleSafetyNetCheck` が `ConsumePendingOverflowFlag()` を検出し即座に VerifyAndRepair を発火 (≤10 秒) |
| DrainPendingRemovals | RAII NodeGuard でスコープ末尾に `fetch_sub + delete` を不可分保証。re-enqueue ループ廃止 (線形増加の主因を排除) |
...
`ProcessPendingRemovals()` は最大 `MAX_DRAIN_PER_TICK` (256) 件/tick でドレインし、残留時は `hWakeupEvent_` を再シグナルして次 tick に継続する。backlog > 8,192 で `LOG_ALERT` を出力する。
...
| `DEGRADED_SCAN_INTERVAL` | 20,000 ms | DEGRADED モードフォールバックスキャン周期 |
...
- 文字列プロパティの読み取りは `propSize` を上限とする bounded copy (§9.15-E)。TDH ペイロードが非終端 (truncated / 不正 provider) の場合でも領域外読みを防止する。未知の `InType` はスキップし、`wchar_t` として再解釈しない。

### 7.3 ヘルスチェック (§9.15)

`IsHealthy()` は IPC スレッドから呼び出される。`stopMtx_` を取得し、Stop() のティアダウンと直列化した上で以下のステートマシンを評価する。

#### 判定フロー

```
running_ == false?  ──→ false (ロック不要: atomic fast path)
sessionHealthy_ == false? ──→ false (ロック不要: atomic fast path)
                               ↓
              ┌── (now - startTime_) < WARMUP_PERIOD_MS (120s)
              │       → true (ウォームアップ猶予)
              └── ウォームアップ経過後
                    ├── lastEventTime_ == 0 → IsTraceSessionAliveLocked() のみ
                    ├── (now - lastEvent) > STARVATION_MS (60s)
                    │     ├── delta lostEventCount_ > LOST_EVENT_THRESHOLD (10) → false
                    │     └── !IsTraceSessionAliveLocked() → false
                    └── その他 → true
```

#### IsTraceSessionAliveLocked()

`ControlTraceW(EVENT_TRACE_CONTROL_QUERY)` でセッションの生存を確認する。結果は `TRACE_CHECK_CACHE_MS` (1 秒) キャッシュし、連続呼び出しの ETW API コストを抑制する。

#### ウォームアップ猶予 (§9.15-A)

`Start()` で `startTime_` を記録し、120 秒間は無条件 healthy を返す。ブートストームからの静穏遷移中に false unhealthy が発生し不要な ETW 再起動が連鎖するのを防止する。

#### イベント枯渇判定 (§9.15-B)

60 秒以上イベントが届いていない場合、まず `lostEventCount_` のデルタ (前回チェックからの増分) を確認する。デルタが閾値 (10) を超過する場合はバッファオーバーフローによる unhealthy。超過しない場合は ETW セッション生存クエリで最終判定する。これにより、idle (正常な無イベント状態) と ETW セッション死を区別する。

#### スレッドセーフティ

`IsHealthy()` は `const` メンバ関数だが、mutable なヘルスチェック状態 (`lastCheckedLost_`, `lastTraceCheckTime_`, `cachedTraceAlive_`) を更新する。これらは `stopMtx_` で保護される。
...
### 7.4 静的インスタンスポインタ (§9.15-C)

ETW コールバックは C API であり、`this` ポインタを受け取れない。`ProcessMonitor::instance_` (`std::atomic<ProcessMonitor*>`) 静的メンバで唯一のインスタンスへのルーティングを行う。`Start()` で release store、`Stop()` で `nullptr` に release store、`EventRecordCallback` で acquire load してローカル `self` にスナップショットする (TOCTOU 回避)。

### 7.5 ETW シャットダウン契約 (§9.15-D)

`Stop()` は以下の順序を厳守する。順序逆転はデッドロックを引き起こす (`ProcessTrace()` は `CloseTrace()` 前に返らないため)。

```
1. stopRequested_ = true          (コールバック早期リターンガード)
2. CloseTrace(traceHandle_)       (ProcessTrace をアンブロック)
3. ControlTrace(STOP)             (セッション終了)
4. consumerThread_.join()          (コールバックドレイン完了待機)
5. instance_ = nullptr / running_ = false  (join 後のみ安全)
```

ステップ 1〜3 は `stopMtx_` ロック下で実行し、IPC スレッドの `IsHealthy()` との race を排除する。ステップ 4 (join) はロック外で実行し、IPC スレッドの不必要なブロックを回避する。

`ControlTraceW(STOP)` の戻り値は `ERROR_SUCCESS`, `ERROR_MORE_DATA`, `ERROR_WMI_INSTANCE_NOT_FOUND` を正常系として扱い、それ以外は `LOG_DEBUG` で記録する。

### 7.6 ETW 信頼性強化 (v1.0.2)
...
| `[Logging]` | `CrashDump` | 0 / 1 | 未処理例外発生時の MiniDump 書き出し (既定=0、無効)。有効化すると `<install dir>\crash\UnLeaf_Service_YYYYMMDD_HHMMSS.sss.dmp` に出力 (§11.6 参照) |
...
## 10. RegistryPolicyManager (v5)

### 10.1 設計概要

RegistryPolicyManager v5 は IFEO (exe 名単位) と PowerThrottle (パス単位) を分離管理する。同一 exe 名の複数パス (Chrome Stable + Canary 等) を独立管理可能。

**設計契約**:
- EngineCore が `CanonicalizePath()` で完全正規化済みパスを生成する責務を持つ
- RegistryPolicyManager は canonical 入力を前提とする
- `NormalizePath()` は防御的処理（不正入力対策）としてのみ残す

**ロック設計**: `policyCs_` → `manifestCs_` (この順序、逆転禁止、同時保持禁止)。`policyCs_` 保持中の I/O はゼロ。

### 10.2 レジストリパス

UnLeaf は 2 つのレジストリ場所に書き込む:

#### A. PowerThrottle (パス単位)
...
> **IFEO グローバルスコープ**: IFEO は exe 名ベースで OS レベルでグローバルに適用される。path target の IFEO は同名の別インストール先にも影響する。パス固有の差別化は PowerThrottle のみで行う。

### 10.3 Per-Entry State Machine

各ポリシーエントリは状態遷移を持つ:

```
APPLYING → (registry write + verify) → COMMITTED
APPLYING → (write/verify failed) → rollback (entry 削除)
```

- `APPLYING`: 他スレッドからの同一パス Apply を exponential backoff で排他
- `COMMITTED`: registry 実体が検証済み。policyMap_ に永続

### 10.4 マニフェスト永続化

レジストリポリシーの適用状態は `UnLeaf_policies.ini` ファイルに v5 形式で永続化する。
...
- **クラッシュ安全性**: `SaveManifestAtomic()` — temp file 書き込み → `FlushFileBuffers` → `MoveFileExW(REPLACE_EXISTING | WRITE_THROUGH)` で原子的に更新
- `stateVersion_` (monotonic atomic counter) による snapshot 整合性チェック付き

### 10.5 プロアクティブポリシー生成

v5 はリアクティブ (ETW 検出時のみ) からプロアクティブ (config 起点) に移行。

```
ApplyProactivePolicies() [Initialize / HandleConfigChange 時]
  ├── path target (IsPathEntry=true)
  │   ├── CanonicalizePath(target.name) → canonical path
  │   ├── FileExistsW(canonical) ?
  │   │   ├── true  → ApplyPolicy(name, path)  [IFEO + PowerThrottle]
  │   │   └── false → ApplyIFEOOnly(name)       [IFEO のみ、PT は ETW フォールバックで適用]
  │   └── desiredNames / desiredPaths に追加
  ├── name-only target
  │   └── ApplyIFEOOnly(name) → desiredNames に追加
  └── ReconcileWithConfig(desiredNames, desiredPaths)
      └── config から除外されたエントリを registry + in-memory から削除
```

**ETW フォールバック / name-only 分岐 (§9.14)**: `ApplyOptimizationWithHandle` は `targetNameSet_` を参照して name-only / path-based を判別する。name-only ターゲット (例: `chrome.exe=1`) は proactive 時に `ApplyIFEOOnly` のみ呼ばれ `policyMap_` に登録されないため、ETW コールバック到達時点で resolvedPath が取得できれば `HasPolicy` をバイパスして無条件に `ApplyPolicy` を呼び出す。path-based ターゲットは従来どおり `HasPolicy` ゲートを使用する。

### 10.6 ライフサイクル

| 操作 | メソッド | 呼び出し元 | 説明 |
|------|---------|-----------|------|
| プロアクティブ適用 | `ApplyProactivePolicies()` | EngineCore::Initialize / HandleConfigChange | config 全エントリへポリシー適用 |
| フルポリシー適用 | `ApplyPolicy()` | ApplyProactivePolicies / ETW fallback | IFEO + PowerThrottle (5 phase) |
| IFEO のみ適用 | `ApplyIFEOOnly()` | ApplyProactivePolicies | name-only target 用 |
| config 整合 | `ReconcileWithConfig()` | ApplyProactivePolicies | stale エントリ削除 (stateVersion_ race 防止) |
| ポリシー存在確認 | `HasPolicy()` | ApplyOptimizationWithHandle | ETW fallback 判定 |
| 正常停止 | `CleanupAllPolicies()` | EngineCore::Stop | インメモリ map ベース |
| 登録解除 | `RemoveAllPolicies()` | Manager (サービス登録解除時) | マニフェストベース |
| 初期化 | `Initialize()` | EngineCore::Initialize | マニフェストロード (クラッシュ復旧) |
| 整合性検証 | `VerifyAndRepair()` | SafetyNet (30分間隔) | registry/memory 不整合を自動修復 |

全操作は **冪等** である。`ifeoRefCount_` で同一 exe 名の複数パスエントリを参照カウント管理し、最後の参照が消えた時点で IFEO キーを削除する。
...
### 11.6 クラッシュダンプ (CrashDump オプトイン)

`src/common/crash_handler.{h,cpp}` に `SetUnhandledExceptionFilter` ベースの MiniDump ライターを実装。**既定では無効**で、`[Logging] CrashDump=1` を明示設定した場合にのみ `ServiceMain`/`RunAsConsole` 冒頭でインストールされる。

| 項目 | 値 |
|------|-----|
| 設定キー | `[Logging] CrashDump=0/1` (既定 `0`) |
| 出力先 | `<install dir>\crash\UnLeaf_Service_YYYYMMDD_HHMMSS.sss.dmp` |
| ダンプ種別 | `MiniDumpWithThreadInfo \| MiniDumpWithIndirectlyReferencedMemory` |
| ファイル名形式 | ミリ秒精度タイムスタンプ (PID なし、単一インスタンスサービス前提) |
| WER との関係 | `EXCEPTION_CONTINUE_SEARCH` を返却し WER / デバッガにチェインするため、MiniDump と WER が両方得られる |
| フィルタ再インストール | `InterlockedCompareExchange` によりプロセスあたり 1 回のみ |
| 旧フィルタ検出 | `SetUnhandledExceptionFilter` の戻り値 (旧フィルタ) を install 時に取得し、非 null の場合のみ `LOG_DEBUG` で記録 (chain は行わない) |

#### 設計ポリシー

- **ポータブル方針堅持**: 既定無効にすることで、オプトインしないユーザーの install フォルダに `crash\` サブフォルダが一切生成されない
- **crash filter は async-signal-safe 相当**: ファイル書き出し以外のヒープ割当・CRT locale・ログ出力を一切行わない (`g_crashDir` は `wchar_t[MAX_PATH]` 固定バッファにキャッシュ)
- **チェインなし**: 旧フィルタを保持していても crash 時には呼び出さない。CRT 破損状態での二次災害を回避し、OS 既定 (WER) に一本化
- **ビルド要件**: `dbghelp.lib` リンク + `/Zi /DEBUG /OPT:REF /OPT:ICF` ビルドフラグ。PDB は Release ビルドにも常時含める

#### インストール箇所

`src/service/service_main.cpp` の `ServiceMainFunc` / `RunAsConsole` で、`UnLeafConfig::Instance().Initialize(baseDir)` 直後に以下を呼び出す:

```cpp
if (UnLeafConfig::Instance().IsCrashDumpEnabled()) {
    InstallCrashHandler(baseDir);
...
ApplyOptimization(pid, name, isChild, parentPid)
  ...
  │
  ├── Registry Policy Fallback (proactive 失敗リカバリ)
  │   ├── resolvedPath 非空の場合:
  │   │   ├── HasPolicy(resolvedPath) → true → skip (proactive 適用済み)
  │   │   └── HasPolicy(resolvedPath) → false → ApplyPolicy(name, resolvedPath)
  │   │       ├── APPLYING state 挿入 (policyCs_ — ZERO I/O)
  │   │       ├── DisablePowerThrottling(path) + SetIFEOPerfOptions(name, 3)
  │   │       ├── registry 実体検証 (IFEOKeyExists + PowerThrottleValueExists)
  │   │       ├── COMMITTED state 遷移 (policyCs_ — ZERO I/O)
  │   │       └── SaveManifestAtomic() (temp + FlushFileBuffers + MoveFileExW)
  │   └── LOG_DEBUG "[REGISTRY] Fallback policy applied"
  │
  ...
  │   └── needsPolicyRetry = resolvedPath.empty()
  │       (true = ETW コールバック時点で QueryFullProcessImageNameW 失敗、SafetyNet でリトライ)
  │
  ├── プロセス終了監視の登録
  ...
  │
  ├── trackedProcesses_ 上限チェック (MAX_TRACKED_PROCESSES = 2,000)
  │   ├── size >= MAX+32 (forceEvict): SelectEvictionCandidates() で全超過分を一括選出
  │   └── evictionCounter & 0xF == 0 (定期): SelectEvictionCandidate() で1件選出
  │       退避候補 → trackedCs_ 解放後に pendingRemovalPids_ へ push
  │       (trackedCs_ 保持中は erase 禁止 — RemoveTrackedProcess() に全委任)
  │
...
RemoveTrackedProcess(pid)
  ...
  │
  │  ──── Phase 5: Job Object エントリ削除 ────
  │
  └── CSLockGuard(jobCs_)          ← jobCs_ のみ (trackedCs_ は既に解放済み)
      └── jobObjects_.erase(pid)   ← unique_ptr dtor が CloseHandle を呼ぶ; root 以外は no-op
```
...
HandleSafetyNetCheck()
  │
  ├── [§9.14 Policy Recovery] CSLockGuard(trackedCs_)
  │   └── needsPolicyRetry=true のプロセスを PolicyRetryInfo ベクタに収集 (PMR arena)
  │
  ├── [§9.14 Policy Recovery] 各 PolicyRetryInfo について:
  │   ├── ResolveProcessPath(hProcess) → resolved (borrowed handle、同一スレッド)
  │   ├── resolved 空 → skip (次回 SafetyNet でリトライ)
  │   ├── HasPolicy(resolved) = false → ApplyPolicy(lowerName, resolved)
  │   │   LOG_INFO "[REGISTRY] SafetyNet policy recovery: ..."
  │   └── CSLockGuard → fullPath = resolved, needsPolicyRetry = false
  │
  ├── CSLockGuard(trackedCs_)
  │   └── STABLE フェーズのプロセス PID を収集
  │       (AGGRESSIVE は遅延検証中、PERSISTENT はタイマー稼働中なのでスキップ)
  │
  ├── pidsToCheck.empty() && policyRetries.empty() → return
  │
  ├── 各 PID について:
...
```
OnProcessStart(pid, parentPid, imageName, imagePath)
  │
  ├── stopRequested_ → return
  ├── IsCriticalProcess(imageName) → return
  │
  ├── isTarget = IsTargetName() || IsTrackedParent() || HasPathTargets()
  │
  └── isTarget → EnqueueRequest(ETW_PROCESS_START, pid, parentPid, imageName, imagePath)
                  ※ ApplyOptimization は呼ばない
                  ※ ETW コールバックスレッドでブロッキング OS 呼び出しを行わない設計
```

```
DispatchEnforcementRequest(ETW_PROCESS_START) [EngineControlLoop 上で実行]
  │
  ├── IsTrackedParent(req.parentPid) → ApplyOptimization(isChild=true, parentPid)
  ├── IsTargetName(req.imageName)   → ApplyOptimization(isChild=false, parentPid=0)
  └── HasPathTargets()              → TryApplyByPath(pid, imageName)
```

**設計変更理由 (v1.1.4)**: `AssignProcessToJobObject` 等の OS 呼び出しが ETW コールバックスレッド上でブロックすると、`consumerThread_.join()` が `EngineControlLoop` を無期限停止させる。ETW コールバックは即時リターンを保証するため、重処理はすべてキュー経由で `EngineControlLoop` に委譲する。
...
| `policyCacheMap_` | `unordered_map<wstring, list<wstring>::iterator>` | LRU エビクション (POLICY_CACHE_MAX_SIZE=1,024) | 最古エントリから自動削除 |

`errorLogSuppression_` は `trackedCs_` で保護される。`PerformPeriodicMaintenance()` が `SUPPRESSION_CLEANUP_INTERVAL` (60s) ごとに TTL 切れエントリを除去し、緊急キャップを `SUPPRESSION_MAX_SIZE` (2,000) で適用する。`policyCacheMap_` は EngineCore 側の重複 Apply 排除用 LRU キャッシュで、`POLICY_CACHE_MAX_SIZE` (1,024) 超過時に最古エントリを自動削除する。
...
| `queueCs_` | EngineCore | `criticalQueue_`, `nonCriticalQueue_` |
| `pendingRemovalCs_` | EngineCore | `pendingRemovalPids_` |
| `targetCs_` | EngineCore | `targetNameSet_`, `targetPathSet_`, `pathTargetFileNames_` |
...
| `policyCs_` | RegistryPolicyManager | `policyMap_`, `ifeoAppliedSet_`, `ifeoRefCount_` (ZERO I/O) |
| `manifestCs_` | RegistryPolicyManager | マニフェストファイル I/O |

### 14.1.1 std::mutex 一覧

| 変数名 | 所属クラス | 保護対象 |
|--------|-----------|---------|
| `stopMtx_` | ProcessMonitor | Stop() ティアダウン (`sessionHandle_`, `traceHandle_`) vs IPC スレッドの `IsHealthy()` / `IsTraceSessionAliveLocked()` を直列化。mutable ヘルスチェック状態 (`lastCheckedLost_`, `lastTraceCheckTime_`, `cachedTraceAlive_`) の排他も兼ねる (§9.15-D) |
...
policyCs_ → manifestCs_ (RegistryPolicyManager — 逆転禁止、同時保持禁止)
```
...
- `jobObjects_`: `map<DWORD, unique_ptr<JobObjectInfo>>` で管理。EngineControlLoop スレッド (`RemoveTrackedProcess`, `RefreshJobObjectPids`) と ETW ConsumerThread (`CreateAndAssignJobObject`) の両方からアクセスされるため、**全アクセスは `jobCs_` 取得が必須**。lock 順序は `jobCs_` → `trackedCs_`
- プロセス終了時のエントリ削除は `RemoveTrackedProcess()` Phase 5 (`jobCs_` 単独) で実施。`trackedCs_` 外で行うことで `RefreshJobObjectPids` との lock inversion を回避
...
| 定数名 | 値 | 説明 |
|--------|-----|------|
| `MAX_TRACKED_PROCESSES` | 2,000 | 最大追跡プロセス数 (engine_core.h) |
| `MAX_PENDING_REMOVALS` | 4,096 | pendingRemoval キュー上限 (engine_core.h) |
| `MAX_DRAIN_PER_TICK` | 256 | ProcessPendingRemovals 1tick最大ドレイン数 (engine_core.h) |
...
| `MAX_PENDING_QUEUE_SIZE` | 512 | PendingRemoval lock-free キュー上限 (registry_manager.h §9.14-B) |
...
| `DEGRADED_SCAN_INTERVAL` | 20,000 | 縮退スキャン間隔 |
...
エンフォースメントキュー制限 (`§9.14-A`):

| 定数名 | 値 | 説明 |
|--------|-----|------|
| `ENFORCEMENT_QUEUE_SOFT_LIMIT` | 4,096 | NON-CRITICAL キュー個別上限 |
| `ENFORCEMENT_QUEUE_HARD_LIMIT` | 8,192 | CRITICAL キュー個別上限 |
| `ENFORCEMENT_QUEUE_TOTAL_LIMIT` | 8,192 | 2キュー合計絶対上限 (`static_assert` でビルド強制) |
| `ENFORCEMENT_CRITICAL_PER_TICK` | 512 | 1 tick CRITICAL 処理上限 (CPU バースト防止) |
| `MAX_SAFETY_SCAN_PER_TICK` | 64 | SafetyNet ラウンドロビン 1 tick スキャン上限 (§9.14-E) |
| `SAFETY_SCAN_BACKSTOP_MS` | 30,000 | SafetyNet 30 秒バックストップ (ETW silent drop 対応, §9.14-E) |
| `PERIODIC_FULL_SCAN_INTERVAL` | 20,000 | NORMAL モード周期 `InitialScan()` 間隔 ms。ETW 状態によらず EcoQoS 検知遅延上限を 20s に保証 (§9.18) |
| `ETW_STALL_CHECK_INTERVAL` | 30,000 | ETW stall 検知チェック間隔 ms (§9.18) |
| `ETW_RESTART_COOLDOWN_MS` | 180,000 | ETW 障害検知 → 再起動の最短間隔 ms。Phase B (障害検知) にのみ適用 (§9.18) |
| `ETW_COLD_DEAD_THRESHOLD_MS` | 240,000 | 起動/再起動後 eventCount=0 が続く cold dead 判定猶予 ms。COOLDOWN+60s の余裕で設計 (§9.18) |
| `ETW_RECOVERY_VERIFY_MS` | 30,000 | 再起動後に eventCount 増加を確認する猶予 ms。超過で retry/DEGRADED 遷移 (§9.18) |
| `ETW_MIN_EVENTS_FOR_CHECK` | 100 | hot stall パス専用の誤検知防止閾値。cold dead パスには不要 (§9.18) |
...
### A.8 ProcessMonitor ヘルスチェック定数 (process_monitor.h, §9.15)

| 定数名 | 値 | 説明 |
|--------|----|------|
| `WARMUP_PERIOD_MS` | 120,000 | セッション開始後のウォームアップ猶予期間 (ms) |
| `STARVATION_MS` | 60,000 | イベント枯渇と判定する無イベント経過時間 (ms) |
| `LOST_EVENT_THRESHOLD` | 10 | チェック間の lost event デルタ閾値 (超過で unhealthy) |
| `TRACE_CHECK_CACHE_MS` | 1,000 | IsTraceSessionAliveLocked() の結果キャッシュ期間 (ms) |

### A.9 レジストリ定数 (registry_manager.h)
...
**ETW 安定性改善 — MatchAnyKeyword=0x30 根本修正 (§9.20)**

| # | 変更 |
|---|------|
| #1 | **`MatchAnyKeyword=0x30` 根本修正**: `Microsoft-Windows-Kernel-Process` プロバイダは `keyword=0` でイベントを発行する (Windows 11 Build 26200 確認)。`MatchAnyKeyword=0x30` (PROCESS\|THREAD) はゼロ値と AND を取ると常に 0 になるため、すべてのコールバックが無音で除外され `DEGRADED_ETW` 遷移を引き起こしていた。`MatchAnyKeyword=0` に変更し ETW デリバリを回復 |
...
| #3 | **ConsumerThread 診断ログ追加**: `OpenTraceW` ハンドル値、`ProcessTrace` 入退出・経過時間 (ms)、初回コールバック受信確認 (`firstCallbackLogged_`)、初回イベント keyword/eventId を記録。事後 ETW 障害分析を支援 |
| #4 | **`firstCallbackLogged_` per-session リセット**: `Start()` で `EnableTraceEx2` 成功後に `false` リセット。セッション境界をまたいだ stale 状態汚染を防止 |
| #5 | **`ResolveProcessPath` ノイズ抑制**: `QueryFullProcessImageNameW` の `error=31` (ERROR_GEN_FAILURE: システムプロセス) と `error=87` (ERROR_INVALID_PARAMETER: 終了済みプロセス) をデバッグ出力から除外。未知エラーのみ出力 |
| #6 | **ログレベル正規化**: `OpenTraceW handle=` / `ProcessTrace enter` / `ProcessTrace exited` を ALERT→INFO に降格 (毎起動・停止で発生する通常ログ)。`Lost event detected` を ALERT→DEBUG に降格 (~1/sec の構造的ノイズ、機能影響なしと確認済み) |
| #7 | **Lost event DEBUG ログ レート制限**: `lastLostLogTime_` (atomic ULONGLONG) による 60 秒デバウンス。イベント量に依らず最大 1 回/分に制限。~1/sec の構造的ノイズによる DEBUG ログ洪水を抑制しつつ観測可能性を維持 |
| #8 | **`[DIAG]` ログに `etwLost=N` フィールド追加**: `lastDiagLostCount_` スナップショットにより DIAG インターバルごとの lost event 増分 (累積値ではなくデルタ) を記録。バースト/スパイク検出を支援 |

> **Note**: `EVENT_FILTER_TYPE_EVENT_ID` フィルタは試行したが有効化後にイベント配信がゼロになるため無効化済み。根本原因未解明 (プロバイダのフィルタ非対応 vs blob format 不正)。EventID 非マッチ時は callback から即リターンするトレードオフを採用。

---

### v1.1.4 (2026-05-01)

**ETW コールバック非ブロッキング化 — クラッシュ根本修正 (§9.19)**

| ID | 内容 |
|----|------|
| #1 | **`OnProcessStart` queue-based 化**: `ApplyOptimization` の ETW コールバックスレッド上での直接呼び出しを廃止。`EnqueueRequest(ETW_PROCESS_START)` のみ実行し、`OpenProcess` / `CreateJobObject` / `AssignProcessToJobObject` 等のブロッキング OS 呼び出しを `EngineControlLoop` 上で処理する。ETW コールバック内ブロック → `consumerThread_.join()` 無期限停止 → `INVALID_PROCESSTRACE_HANDLE` ACCESS VIOLATION クラッシュ（ダンプ実例: 9 分ブロック × 2 回）を根本修正 |
| #2 | **`EnforcementRequest` 構造体拡張**: `parentPid` / `imageName` / `imagePath` フィールドを追加。`ETW_PROCESS_START` 専用コンストラクタ `(DWORD pid, DWORD parent, const wstring& name, const wstring& path)` を追加。既存 `(DWORD p, EnforcementRequestType t, uint8_t step=0)` コンストラクタは互換維持 |
...
| #4 | **`queueCs_` 設計契約コメント追加**: `// ZERO-I/O, no blocking — deque ops only` を宣言に付与 |

### v1.1.3 (2026-04-28)

**ETW 監視安定化・Windows 11 Update 後 EcoQoS 解除不全対策 (§9.18 拡張)**

| ID | 内容 |
|----|------|
| #1 | `ScanRunningProcessesForMissedTargets`: 全走査完了時 (pass1 自然終了 / pass2 完了) に `lastScannedPid_=0` を強制セット。次 tick で必ず PID 0 から再走査し、高 PID 張り付きによる低 PID ターゲット (早期起動 chrome.exe 等) の継続的な取りこぼしを解消 |
| #2 | **`PERIODIC_FULL_SCAN_INTERVAL` 60 s → 20 s**: NORMAL モードの `InitialScan()` 周期を短縮。根本要件「EcoQoS を OFF にする」は ETW 状態によらず保証すべきであり、ETW 破損中 (cold-dead 検知前の最大 4 分) でも新規 Chrome EcoQoS 遅延を ≤ 20 s に抑制する。リソース影響: ToolHelp32 scan ~1-5 ms × 3回/分、追加 CPU ≈ 0.007 %（§9.11 違反なし）、メモリ増加なし（Snapshot は都度解放）|
| #3 | `[DIAG]` 診断ログに `etwEvents=%u` フィールドを追加 (`processMonitor_.GetEventCount()`)。ETW デリバリ健全性の事後検証を可能にする |
| #4+#5 | **ETW 障害検知拡張 — 2系統トリガー + 3ステート機械**: 従来の hot stall 検知 (eventCount ≥ 100 かつ delta=0) に加え、**cold dead 検知** (起動/再起動後 eventCount=0 が `ETW_COLD_DEAD_THRESHOLD_MS=240 s` 継続) を追加。`IsHealthy()` は `ControlTrace(QUERY)` 成功なら healthy と誤判定するため cold dead を見逃す盲点を補完。再起動後は `EtwState::VERIFYING_RECOVERY` に遷移し `ETW_RECOVERY_VERIFY_MS=30 s` ウィンドウ内に eventCount 増加を確認。増加なければ再試行 (最大 2 回)、2 回失敗で `DEGRADED_ETW` 遷移。`etwVerificationBaseCount_` で Start() 直後の基準値を固定し hotStall 後の lastEtwEventCount_ 汚染を回避 |

新規ヘルパー:
- `bool HasAnyTargetRunning() const` — `trackedProcesses_` fast path を先行チェックし、miss 時のみ `Toolhelp32Snapshot` にフォールバック。ETW stall/cold-dead ガードの誤再起動防止に使用
- `bool RestartETW()` — `ProcessMonitor.Stop() → Sleep(50ms) → Start()` + state 更新 (`etwState_`, `etwVerificationBaseCount_`, `etwRestartVerificationDeadline_`) を集約

**`EtwState` 3ステート機械**:

| 状態 | 意味 | 遷移条件 |
|------|------|---------|
| `HEALTHY` | 正常・通常監視 | 初期値 / 復帰確認成功 / 対象プロセス消滅 |
| `VERIFYING_RECOVERY` | 再起動後の復帰確認中 | `RestartETW()` 成功後 |
| `DEGRADED` | ETW 恒久障害 → SafetyNet 一次運用 | `ETW_RECOVERY_VERIFY_MS` タイムアウト × 2回 |

**パラメータ変更**:

| 定数 | 変更前 | 変更後 | 理由 |
|------|--------|--------|------|
| `PERIODIC_FULL_SCAN_INTERVAL` | 60,000 ms | **20,000 ms** | 根本要件: ETW 状態によらず EcoQoS ≤20s 保証 |
| `DEGRADED_SCAN_INTERVAL` | 30,000 ms | **20,000 ms** | DEGRADED 中の新 Chrome 検知遅延を 30s→20s に短縮 |
| `ETW_COLD_DEAD_THRESHOLD_MS` | (新規) | **240,000 ms** | cold-dead 判定猶予。`ETW_RESTART_COOLDOWN_MS`(180 s) より長く設定 |
| `ETW_RECOVERY_VERIFY_MS` | (新規) | **30,000 ms** | 再起動後 eventCount 増加確認ウィンドウ |

障害ケース対応マトリクス (変更後):

| 障害ケース | 最大補正時間 | 機構 |
|---|---|---|
| SafetyNet PID 張り付き | 〜30 秒 | #1 |
| ETW silent drop (chrome のみ不着) | **〜20 秒** | #2 PERIODIC_FULL_SCAN (20 s) |
| ETW stall (session alive, events 停止) | 〜30 秒 + 3 分 cooldown | #4+#5 hot stall |
| ETW cold dead (起動直後から events なし) | 〜240 秒 + 30 s×2 verify | #4+#5 cold dead |
| ETW 完全停止 (session dead) | 〜90 秒 | 既存 `IsHealthy()` |
...
| # | 内容 |
|---|------|
| FIX-1 | `RemoveTrackedProcess()` 末尾に `jobObjects_.erase(pid)` を追加。`trackedCs_` 解放後に `jobCs_` 単独で取得し、`unique_ptr<JobObjectInfo>` のデストラクタが `CloseHandle(jobHandle)` を呼ぶことを保証。lock inversion (`jobCs_` → `trackedCs_` 順序制約) を回避 |

**ヒープ断片化抑制 (P1/P2)**

| # | 内容 |
|---|------|
| OPT-1 | `ParseProcessStartEvent()` の TDH バッファ (`buffer`, `propData`) を `thread_local std::vector<BYTE>` (`s_tdhBuffer`, `s_propBuffer`) に置換。capacity は縮小せず、ETW イベントごとの反復 alloc/free を排除。`_DEBUG` 時に `capacity > MAX_TDH_BUFFER * 2` を `LOG_DEBUG` で検知 |
| OPT-2 | `ProcessMonitor` ctor / `Start()` / `Stop()` の `std::wstringstream` 4箇所を `std::to_wstring()` に置換 |

**デバッグ品質強化**

| # | 内容 |
|---|------|
| DBG-1 | `engineControlThreadId_` を `DWORD` から `std::atomic<DWORD>` に変更 (`memory_order_relaxed`)。`EngineControlLoop()` 先頭/末尾で `store()`。`RemoveTrackedProcess` / `RefreshJobObjectPids` / `ProcessPendingRemovals` の DEBUG ASSERT で `load(relaxed)` により可視性を保証 |

**ヒープメモリ管理強化 (§9.17-B)**

| # | 内容 |
|---|------|
| HEAP-1 | `EngineCore::Start()` 冒頭で `HeapSetInformation(GetProcessHeap(), HeapOptimizeResources, ...)` を呼び出し。プロセスヒープが idle 時に未使用コミットページを積極的にデコミットするよう Windows に指示。Windows 8.1+ ベストエフォート (失敗は無視)。6.5 時間テストで Private Bytes の増加量が前バージョン比で約 5 倍改善 (3.78→6.26 MB / 6.5h) を確認 |
...
| § | 内容 |
|---|------|
| §9.15-A | **ウォームアップ猶予期間 (120s)** — `Start()` で `startTime_` を記録。ブートストーム→静穏遷移中の false unhealthy を抑制し不要な ETW 再起動の連鎖を防止 |
| §9.15-B | **イベント枯渇判定リワーク** — 60s 無イベント時に (1) lost event デルタ閾値チェック (2) `ControlTraceW(QUERY)` によるセッション生存確認 (1s キャッシュ) の2段判定。idle と ETW セッション死を区別 |
| §9.15-C | **`instance_` を `std::atomic<ProcessMonitor*>` 化** — `EventRecordCallback` でローカル `self` にスナップショットし TOCTOU 排除。acquire/release ペアリング |
| §9.15-D | **ETW シャットダウン契約の明文化** — `stopRequested_` → `CloseTrace` → `ControlTrace(STOP)` → `join` → `instance_ clear` の5ステップ順序を DO NOT REORDER としてコメント化。`stopMtx_` で Stop() vs IsHealthy() を直列化 |
| §9.15-E | **TDH パースの bounded copy** — `ParseProcessStartEvent` で文字列プロパティ読み取りを `propSize` 上限の bounded copy に置換。非終端 TDH ペイロードによる領域外読み (AV リスク) を排除。未知 InType はスキップ |
| §9.15-F | **`Start()` での状態リセット** — `eventCount_` / `lostEventCount_` / `lastEventTime_` / `startTime_` / `lastCheckedLost_` / `cachedTraceAlive_` を明示リセット。サービス再起動後の残留状態汚染を防止 |
| §9.15-G | **`ControlTrace(STOP)` 戻り値検査** — `ERROR_SUCCESS` / `ERROR_MORE_DATA` / `ERROR_WMI_INSTANCE_NOT_FOUND` を正常系として扱い、それ以外は `LOG_DEBUG` で記録 |
| §9.15-H | **Stop() 時の health cache 無効化** — `cachedTraceAlive_ = false` / `lastTraceCheckTime_ = 0` をクリアし、再 Start() 後の IsHealthy() が前回サイクルの stale "alive" を返すのを防止 |

**CrashDump オプトイン機能 (Phase 2)**

| # | 内容 |
|---|------|
| CRASH-1 | `src/common/crash_handler.{h,cpp}` 新設。`SetUnhandledExceptionFilter` + `MiniDumpWriteDump(MiniDumpWithThreadInfo \| MiniDumpWithIndirectlyReferencedMemory)` ベース。既定無効。`[Logging] CrashDump=1` で opt-in |
| CRASH-2 | 出力先は `<install dir>\crash\UnLeaf_Service_YYYYMMDD_HHMMSS.sss.dmp` (ポータブル方針のため `%ProgramData%` 等を使用しない) |
| CRASH-3 | `UnLeafConfig` に `crashDumpEnabled_` メンバ + `IsCrashDumpEnabled()` アクセサ追加。既定 `false` |
| CRASH-4 | `ServiceMain`/`RunAsConsole` で config 初期化直後に opt-in ゲート付きで `InstallCrashHandler()` 呼出 |
| CRASH-5 | `SetUnhandledExceptionFilter` 戻り値 (旧フィルタ) を install 時に取得し、非 null の場合のみ `LOG_DEBUG` で記録。crash 時のチェインは行わず `EXCEPTION_CONTINUE_SEARCH` で WER に委譲 |
| CRASH-6 | `dbghelp.lib` リンク + `/Zi /DEBUG /OPT:REF /OPT:ICF` ビルドフラグを Release にも適用し PDB を常時出力 |

### v1.1.0 (2026-03-30)

**メモリ安定化・長期稼働品質改善 (§9.00〜§9.09)**

| § | 内容 |
|---|------|
| §9.00 | `trackedProcesses_` ハード上限 (MAX_TRACKED_PROCESSES=2000) 導入。`SelectEvictionCandidate()` 追加 (zombie 優先 → 最古優先)。Logger `WriteMessage()` スタックバッファ最適化 (2048 bytes) |
| §9.01 | `MAX_PENDING_REMOVALS` (4096) 導入。pendingRemoval saturation guard。PMR fallback 検出 (`CountingResource`) |
| §9.02〜§9.04 | eviction 内の `erase` 完全撤廃。`RemoveTrackedProcess()` 一元委任。nested lock 禁止。always push 化 |
| §9.05 | `CountingResource` を anonymous namespace に一元定義 (ODR 違反排除)。`SelectEvictionCandidate()` に DEBUG ASSERT 追加 |
| §9.06〜§9.08 | `SelectEvictionCandidates()` 追加 (burst 一括退避、`partial_sort` 最適化)。dedup O(N) 削除 (`RemoveTrackedProcess` idempotent)。off-by-one 修正 |
| §9.09 | pendingRemoval overflow 時 pop 禁止→LOG_ALERT 化。PMR arena 化 (16KB stack buffer)。`ProcessPendingRemovals` ドレイン制限 (MAX_DRAIN_PER_TICK=256) + runaway 検知 (>8192) |

**RegistryPolicyManager v5 + CPU 暴走対策 + プロアクティブポリシー (§9.10〜§9.13)**

| § | 内容 |
|---|------|
| §9.10 | RegistryPolicyManager v5 全面再設計。IFEO/PT 分離、per-entry state machine (APPLYING→COMMITTED)、lock-free Treiber stack、policyCs_ ZERO I/O、ifeoRefCount_ 参照カウント、VerifyAndRepair (30分)、SaveManifestAtomic、LRU キャッシュ |
| §9.11 | CPU 暴走対策。ProcessPendingRemovals の無条件 SetEvent を hasRemaining 条件付きに修正。全 push サイト (P1-P3) を wasEmpty パターンに統一。EngineControlLoop にスピン検知 (10,000 回超で 1ms sleep) 追加 |
| §9.12 | プロアクティブポリシー生成。リアクティブ→プロアクティブ (config 起点) へアーキテクチャ変更。ApplyProactivePolicies / ApplyIFEOOnly / ReconcileWithConfig 新設。ResolvePathByHandle 完全廃止→CanonicalizePath 置換。HasPolicy + ETW フォールバック。FileExistsW によるパス存在判定 |
| §9.13 | CanonicalizePath 正規化統一。types.h に free function 化。GetFullPathNameW 2段階呼び出し (動的バッファ、長パス対応)。policyMap_ 全キーを CanonicalizePath に統一。NormalizePath はフォールバック/ログ用途に限定。設計契約コメント追加 |

**PowerThrottle 遅延適用バグ修正 (§9.14)**

| § | 内容 |
|---|------|
| §9.14 | `ApplyOptimizationWithHandle` に name-only / path-based 分岐を追加。name-only ターゲットは `targetNameSet_` で判別し `HasPolicy` をバイパスして即座に `ApplyPolicy` を呼び出す。`TrackedProcess` に `needsPolicyRetry` フラグを追加。SafetyNet (10s) に policy recovery ループを追加 — `needsPolicyRetry=true` プロセスの ResolveProcessPath を再試行し成功時に `ApplyPolicy` を呼び出す。SafetyNet PMR arena を 4 KB → 8 KB に拡張。診断ログ追加 (`[DIAG]` タグ): `QueryFullProcessImageNameW` 失敗、パス解決失敗、空パス遅延 |

**ServiceEngine メモリ増加対策 (§9.14 Rev.17)**

| § | 内容 |
|---|------|
| §9.14-C | `ScheduleDeferredVerification` — `std::exchange` + `INVALID_HANDLE_VALUE` 同期待機で旧タイマーハンドル・コンテキストリークを根絶 |
| §9.14-B | `EnqueuePendingRemoval` — CAS ベース上限ガード + overflow → `pendingOverflowFlag_` 即セット。`DrainPendingRemovals` — RAII NodeGuard + re-enqueue ループ廃止（線形増加の主因を排除） |
| §9.14-A | `enforcementQueue_` を `criticalQueue_ + nonCriticalQueue_`（各 std::deque）に分離。TOTAL_LIMIT 絶対保証 + `static_assert` ビルド強制 + 512 件/tick バースト制限 |
| §9.14-E | SafetyNet: CRITICAL drop 差分検出 + 30 秒バックストップ (ETW silent drop 対応) → `ScanRunningProcessesForMissedTargets`（2 パス + `std::max` 単調増加保証） |
| §9.14-H | `PerformPeriodicMaintenance` に 60 秒 `[DIAG] crit= nc= pending= drop= critDrop= critEvict= recovered= tracked=` 追加 |
| §9.14-D | `ReconcileWithConfig` 先頭で `ifeoRefCount_.clear()` + policyMap_ 全走査再構築 |
| §9.14-F | eviction ロジック簡略化 — 常時最大 16 件/挿入でキャップ超過を即処理 |
| §9.14-G | `HandleEnforceError` — insert 直後に `SUPPRESSION_MAX_SIZE/2` を即キャップ |

### v1.0.3 (2026-03-25)

**ログローテーション完全安定化 (§8.58〜§8.61)**

| § | 内容 |
|---|------|
| §8.58 | Manager ログ表示を RichEdit から仮想 ListView に刷新 (UICallback → LogEngine/LogQueue 方式)。自動スクロール停止・ブランクライン消滅 |
| §8.59 | `CheckRotation()` を純粋 `RotationResult` 返却関数に改設計。`SafeInternalLog()` 新設。`FILE_SHARE_DELETE` 追加。`RotationMutexGuard` RAII 化 |
| §8.60 | `MoveFileExW(REPLACE_EXISTING)` → `SetFileInformationByHandle(FileRenameInfoEx, POSIX_SEMANTICS)` に変更。Manager が宛先ハンドルを保持したままローテーション成功 |
| §8.61 | `Global\UnLeafLogRotated` Named Event 追加。Manager の stale handle 検出を WriteFile 前に移動。ログシーケンス逆転バグを根絶 |

### v1.0.2 (2026-03-16)

**ETW 強化・Manager UI 刷新 (§8.50〜§8.57)**

| § | 内容 |
|---|------|
| §8.50 | ETW バッファ設定を明示化 (BufferSize=64KB / MinimumBuffers=4 / MaximumBuffers=32)。zombie セッションクリーンアップ追加 |
| §8.51〜§8.54 | ETW lost event 検出・LOG_ALERT 出力。ウィンドウ位置・サイズ永続化 (UnLeaf.ini `[Manager]`) |
| §8.55〜§8.57 | カラー精度改善。Manager 操作ログのファイル永続化 (UICallback → logQueue_)。`RefreshLogDisplay` 起点位置修正 |
